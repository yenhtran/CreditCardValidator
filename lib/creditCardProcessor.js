    ...
    transactions.forEach(function(transaction) {
        ...
        if ((action === 'Add' || action === 'add' || action === 'ADD') && !findAccount(name)) {
            ...
            if (findAccount(name)) {
                ...
                if ((action === 'Charge' || action === 'charge' || action === 'CHARGE')  && cardValidator.lunh10Validate()) {
                    currentCreditCard.charge(amount)
                } else if ((action === 'Credit' || action === 'credit' || action === 'credit')  && cardValidator.lunh10Validate()) {
